...
def run(file, folder='tmp', base_GRN=None, n_jobs=64):
    ...
    if 'tumor' not in adata.obs['tumor'].unique():
        ...
        with open('tmp/celloracle/skipped_samples.txt', 'a') as f:
...
if __name__ == '__main__':
    ...
    with open('tmp/celloracle/skipped_samples.txt', 'w') as f:
    ...
    completed_samples = set(f.replace('_grn.csv', '.h5ad') 
                          for f in os.listdir('tmp/celloracle') 
    ...
    if os.path.exists('tmp/celloracle/all_grn.csv'):
        existing_results = pd.read_csv('tmp/celloracle/all_grn.csv')
